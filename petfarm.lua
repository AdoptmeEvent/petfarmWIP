...
-- NEW: Helper function to create a temporary, large, visible, collidable platform at a target CFrame
local function createTemporaryLandingPlatform(cframe)
    local tempPlatform = Instance.new("Part")
    tempPlatform.Name = "LandingPlatform"
    tempPlatform.Size = Vector3.new(100, 5, 100) -- Large enough for a safe landing zone
    tempPlatform.Transparency = 0 -- Visible as requested
    tempPlatform.Color = Color3.fromRGB(50, 255, 50) -- Visible bright color for target location
    tempPlatform.Material = Enum.Material.Neon
    tempPlatform.CanCollide = true
    tempPlatform.Anchored = true
    CFrame.new(0, -5, 0) -- Position it slightly below the target CFrame
